...
contract("TOCToken", function (accounts) {
  ...
  it("initializes the contract with the correct values", function () {
    return TOCToken.deployed()
      ...
      .then(function (name) {
        assert.equal(name, "The Open Chain", "has the correct name");
        return tokenInstance.symbol();
      })
      .then(function (symbol) {
        assert.equal(symbol, "TOC", "has the correct symbol");
        return tokenInstance.standard();
      })
      .then(function (standard) {
        assert.equal(standard, "TOC Token v1.0", "has the correct standard");
      });
  });
